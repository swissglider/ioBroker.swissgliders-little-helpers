...
export class NativeSGElementRemoverHelper extends Helper {
            ...
            if (
                ...
                if (result !== undefined) {
                    ...
                    this.adapter.sendTo(
                        obj.from,
                        obj.command,
                        { error: 'unknown error while trying to stop deleteNativeElements' },
                        obj.callback,
                    );
                }
            }
     ...
     */
    private async deleteNativeElements(obj: ioBroker.Message | undefined): Promise<void> {
        if (
            obj &&
            obj !== null &&
            obj !== undefined &&
                ...
                if (
                    ...
                    try {
                        for (const entry of obj.message.message.entries) {
                            const temp = entry.split('.').reduce((o: any, i: any) => o[i], value.native);
                            if (temp !== null && temp !== undefined) {
                                deleteOnElement.push(
                                    `<b>${entry}</b> <span style="color:black">delete on</span> <b>${key}</b>`,
                                );
                                const path = entry.split('.');
                                const toDeleteParam = path.pop();
                                const hal = path.reduce((o: any, i: any) => o[i], value.native);
                                delete hal[<string>toDeleteParam];
                                toDelete = true;
                            }
                        }
                    } catch (e) {
            ...
            if ('callback' in obj && obj.callback && obj.callback !== null && obj.callback !== undefined) {
                const result = {
                    ...
                    <pre>${JSON.stringify(
                        ...
                        null,
                        3,
                    )}</pre><hr>from the following types: <br><pre>${JSON.stringify(
                        obj.message.message.types,
                        null,
                        3,
                    )}</pre><hr>Changes done on the following objects: <br><pre>${JSON.stringify(
                        deleteOnElement,
                        null,
                        3,
                    )}</pre>`,
