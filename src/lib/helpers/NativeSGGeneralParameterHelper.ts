import { Helper } from './IHelper';

interface INativeSwissgliderGeneral {
    adapterName?: string;
    instanceName?: string;
    instanceNunber?: number;
    deviceName?: string;
    channelName?: string;
    stateName?: string;
    id?: string;
    displayName?: string;
    enumName?: ioBroker.StringOrTranslated;
    parentName?: ioBroker.StringOrTranslated;
    orgMembers?: string[];
}

const exclutionsList: string[] = [
    '0_userdata.0',
    'admin.0',
...
export class NativeSGGeneralParameterHelper extends Helper {
    private allDevicesChannelsStatesEnums: { [key: string]: ioBroker.Object } = {};
    ...
    public async onMessage(commandName: string, obj: ioBroker.Message): Promise<boolean> {
        //TODO
        try {
            ...
            if (commandName === 'stopListeningToAll') {
                ...
                this.adapter.sendTo(
                    obj.from,
                    obj.command,
                    { title: 'NativeSGGeneralParameterHelper.stopListeningToAll', message: returnMsg },
                    obj.callback,
                );
                return true;
                ...
                this.adapter.sendTo(
                    obj.from,
                    obj.command,
                    { title: 'NativeSGGeneralParameterHelper.startListeningToAll', message: returnMsg },
                    obj.callback,
                );
                return true;
                ...
                this.adapter.sendTo(
                    obj.from,
                    obj.command,
                    { title: 'NativeSGGeneralParameterHelper.writtingAllGeneralparameter', message: returnMsg },
                    obj.callback,
                );
                return true;
            }
     ...
     *
     */
    private async updateObjectsFor_NativeSwissgliderGeneralParameters(): Promise<void> {
        try {
            ...
            this.adapter.log.error(
                'Error on NativeSGGeneralParameterHelper:updateObjectsFor_NativeSwissgliderGeneralParameters',
            );
            this.adapter.log.debug(error.name);
            this.adapter.log.debug(error.message);
            this.adapter.log.debug(error.stack);
     ...
     */
    private async updateObjectFor_NativeSwissgliderGeneralParameters(
        id: string,
        value: ioBroker.Object | undefined | null,
    ): Promise<void> {
        if (id !== undefined) {
            if (value) {
                // to change
     ...
     * subscribe to all
     */
    private async subsrcibeToAll(): Promise<void> {
        try {
     ...
     * unsubscribe to all
     */
    private async unsubsrcibeToAll(): Promise<void> {
        try {
     ...
     */
    private listenToSubscribtions(): void {
        this.adapter.on('objectChange', async (id: string, obj) => {
            try {
                this.allDevicesChannelsStatesEnums = await this.getAllDevicesChannelsStatesEnums();
                if (obj) {
     ...
     */
    private async writNativeSGGeneralParameters(id: string, obj: ioBroker.Object): Promise<void> {
        try {
            if (obj) {
                // test if needed
                ...
                // The state was changed
                const oldSwissgliderGeneralStruct: INativeSwissgliderGeneral = <ioBroker.Object>(
                    obj.native?.swissglider?.general
                )
                    ? obj.native.swissglider.general
                    : undefined;
                const dStart1 = new Date();
                const newSwissgliderGeneralStruct: INativeSwissgliderGeneral = await this.getINativeSwissgliderGeneralbyObject(
                    id,
                    obj,
                );
     ...
     */
    private async getINativeSwissgliderGeneralbyObject(
        id: string,
        obj: ioBroker.Object,
    ): Promise<INativeSwissgliderGeneral> {
        const newSwissgliderGeneralStruct: INativeSwissgliderGeneral = {};
        ...
        while (tmpID.lastIndexOf('.') != -1) {
            ...
            if (
                ...
                if (tmpObject.type === 'device') {
                    newSwissgliderGeneralStruct.deviceName = <string>tmpObject.common.name;
                }
                if (
                    tmpObject.type === 'channel' &&
                    <string>tmpObject.common.name !== 'Sensors and switches' &&
                    <string>tmpObject.common.name !== 'Lights'
                ) {
                    newSwissgliderGeneralStruct.channelName = <string>tmpObject.common.name;
                }
        ...
        if (
            ...
            if (
                'channelName' in newSwissgliderGeneralStruct &&
        ...
        if (obj.type !== 'enum') {
            if (obj.common.name && !(obj.common.name === null) && !(obj.common.name === undefined)) {
                if (typeof obj.common.name === 'string') {
                    ...
                    this.adapter.log.error(
                        `Statename for ${id} could not be set name: ${JSON.stringify(obj.common.name)}`,
                    );
                }
            }
            ...
            if (
                ...
                if (tmpDN && tmpDN !== null && tmpDN !== undefined) newSwissgliderGeneralStruct.displayName = tmpDN;
            }
        } else {
            if (obj.common.name !== undefined) newSwissgliderGeneralStruct.enumName = obj.common.name;
