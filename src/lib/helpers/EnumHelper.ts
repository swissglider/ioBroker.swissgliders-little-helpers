...
export class EnumHelper extends Helper {
     ...
     *
     */
    private async getEnumConfiguration(obj: ioBroker.Message): Promise<void> {
        ...
        if (obj !== null) {
            ...
            if (enums !== null && enums !== undefined) {
                const new_enums: { [key: string]: any } = {};
     ...
     */
    private async overwritteAllEnums(obj: ioBroker.Message): Promise<void> {
        ...
        if (
            ...
            for (const [key, value] of Object.entries(new_enums)) {
                ...
                if (!tempEG || tempEG === null || tempEG === undefined) {
                    const tmpEnumObject: ioBroker.Enum = {
                        _id: key,
                        common: {
                ...
                if (value && value !== undefined && typeof value === 'object') {
                    for (const [keyE, valueE] of Object.entries(value)) {
                        let tempE = <ioBroker.Enum>await this.adapter.getForeignObjectAsync(keyE);
     ...
     * delete all enums !!!!
     */
    private async deleteAllEnums(obj: ioBroker.Message | undefined): Promise<void> {
        const enums = await this.adapter.getEnumsAsync('');
        if (enums !== null && enums !== undefined) {
